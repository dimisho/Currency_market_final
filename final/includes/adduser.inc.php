...
if (is_empty($_POST)){
    ...
    header("location: ../adduser.php?error=empty");
    exit();
}
if (is_email($_POST["user_email"])){
    $_SESSION["is-invalid"] = true;
    header("location: ../adduser.php?error=not-email");
    exit();
}
if (user_exist($_POST["user_email"])){
    $_SESSION["is-invalid"] = true;
    header("location: ../adduser.php?error=email_exist");
    exit();
}
if ($user_pass != $sub_pass)
{
    $_SESSION["is-invalid"] = true;
    header("location: ../adduser.php?error=pass_not_confirm");
    exit();
}
